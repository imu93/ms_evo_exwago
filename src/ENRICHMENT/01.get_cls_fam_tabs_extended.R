...
for (exp in names(dgeList)) {
  ...
  categories = c("DNA.*As|^RC.*As", "DNA.*S$|^RC.*S$", "^LINE.*As$|^PLE.*As|^Retrop.*As", "^LINE.*S$|^PLE.*S|Retrop.*S", "LTR.*As", "LTR.*S$",
                 "Unk", "Low|Simple|Sat|SINE|MITE", "lincRNA_As", "lincRNA_S", "miRNA_S", "miRNA_As",
                 "*s_rRNA_S|LSU.*_S|^rRNA_S", "*s_rRNA_As|LSU.*_As|^rRNA_As","^tRNA_S", "tRNA_As", 
                 "yRNA|snRNA|snoRNA|other_ncRNA|piRNA|lncRNA|circRNA", "exons_As", "exons_S", "introns_As", "introns_S", "intergenic")
  
  names(categories) = c("DNA_As", "DNA_S", "LINE_As", "LINE_S", "LTR_As", "LTR_S", "Unknown", 
                        "Other_repeat", "lincRNA_As", "lincRNA_S", "miRNA_S", "miRNA_As",
                        "rRNA_S", "rRNA_As","tRNA_S", "tRNA_As", 
  ...
  outFile2 = paste0("~/storage/Data/ms_evo_exwago/analysis/de_tables/ip_cls_tabs/", exp,".cls_prop_exted.txt")
