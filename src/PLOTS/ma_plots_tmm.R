...
for (i in names(de_tbls)) {
  ...
  tmp.tbl$col = ifelse(tmp.tbl$cl == -1, "#C0C0C0", tmp.tbl$col)
  tmp.tbl$ord = ifelse(tmp.tbl$col == "#C0C0C0", 1, 2)
  tmp.tbl$col = ifelse(grepl("s_rRNA_S|LSU_S", rownames(tmp.tbl)), "red", tmp.tbl$col)
...
p1 = ggplot(df2plot, aes(x=Ave_CPM, y=logFC, colour = col)) +
  ...
  scale_color_manual(values = adjustcolor(c("#155AA3","#C0C0C0", "red"), alpha.f = .6), 
                     labels=c('exWAGO enriched', 'not-DE', "rRNA"), name=NULL) +
...
ggsave("nematode_exwago_vs_total_tmm_2.png", plot = p1, device = "png",
