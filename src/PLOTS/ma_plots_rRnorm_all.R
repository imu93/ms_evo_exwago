...
for (i in names(de_tbls)) {
  ...
  tmp.tbl$exp = sub("_", ". ", i)
  tmp.tbl$col = "#C0C0C0"
  tmp.tbl$col = ifelse(tmp.tbl$cl == 1, "#155AA3", tmp.tbl$col)
  tmp.tbl$col = ifelse(grepl("_rRNA_S|LSU_S", rownames(tmp.tbl)), "red", tmp.tbl$col)
...
ggsave("nematode_exwago_vs_total_rRNA_all.png", plot = p1, device = "png",
