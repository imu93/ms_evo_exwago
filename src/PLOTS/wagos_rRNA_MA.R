...
for (i in names(de_tbls)) {
  ...
  tmp.tbl$col %>% table
  tmp.tbl$col = "#C0C0C0"
  tmp.tbl$col = ifelse(tmp.tbl$cl == 1, "#15C1FB", tmp.tbl$col)
...
df2plot$col = factor(df2plot$col, levels = c("#C0C0C0", "#155AA3", "red"))
...
ggsave("wagos_vs_input_rrna.png", plot = p1, device = "png",
