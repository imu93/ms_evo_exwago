...
inFile_1 = "heligmosomoides_bakeri.counts.1827.txt.gz" # main table
...
counts = counts[,grepl("Adult_IP_polyP|Adult_total_poly", colnames(counts))]
...
plotMDS(dge , col=colors[dge$samples$group], 
        pch = 16, cex=3,main="MDS plot of Heligmosomoides bakeri exWAGO vs Adult")
plot.new()
...
contrast = makeContrasts("Adult_IP_polyP-Adult_total_poly",levels = dge$design)
...
write.table(topTable, paste0(outPath_1, "heligmosomides_bakeri.exwago_ip_vs_total_fdr05_lfc1_tmm_topTbl.txt"), 
...
de_tbl$exp = "H. bakeri IP vs Adult total"
de_tbl$exp = factor(de_tbl$exp)
levels(de_tbl$exp) = c(expression(paste(italic("A. ceylanicum "),
                                        "exWAGO IP vs Adult total")))

write.table(de_tbl ,paste0(outPath_1, "heligmosomoides_bakeri.exwago_ip_vs_total_fdr05_lfc1_tmm_deTbl.txt"), 
            col.names = T, row.names = T, sep = "\t", quote = F)
...
p = ggplot(de_tbl, aes(x=Ave_CPM, y=logFC, colour = col)) +
  ...
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), 
                           "inches"))
p
saveRDS(dge,paste0(outPath_7,"heligmosomoides_bakeri.exwago_ip_adult_tmm_dge.Rds"))
...
cts_1 = ggplot(df, aes(x=Ave_CPM, y=logFC, colour = col)) +
  ...
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), 
                           "inches"))
ggsave("heligmosomoides_bakeri.tmm_normcats.png", device = png, plot = cts_1, width = 14, height = 5, dpi = 300,
...
write.table(df, paste0("~/storage/Data/ms_evo_exwago/analysis/de_tables/tmm_norm_cats/",
                       "heligmosomoides_bakeri.tmm_nomcats.txt"), sep = "\t", quote = F, col.names = T, row.names = T)
...
outToptable = paste0(outPath_2, "heligmosomoides_bakeri.exwago_IP_adult_lfc1_fdr01_rRNA_topTbl.txt")
...
de_tbl_norm$exp = "H. bakeri IP vs Adult total"
de_tbl_norm$exp <- factor(de_tbl_norm$exp)
ncont = "IP vs Adult total"
levels(de_tbl_norm$exp) <- c(expression(paste(italic("H. bakeri "))))


outDEtbl = paste0(outPath_2, "heligmosomoides_bakeri.exwago_IP_adult_lfc1_fdr01_rRNA_deTbl.txt")
...
saveRDS(dge,paste0(outPath_3, "heligmosomoides_bakeri.ip_vs_adult_rRNA_norm_dge.Rds"))
...
write.table(cameraRes, 
            paste0(outPath_4,"heligmosomoides_bakeri.IP_camera_enr_class_simple_rRNA.txt"),
...
write.table(cameraRes, 
            paste0(outPath_5,"heligmosomoides_bakeri.IP_camera_enr_family_simple_rRNA.txt"),
            sep = "\t", row.names = T, col.names = T, quote = F)
