...
inFile_1 = "nippostrongylus_brasiliensis.counts.1827.txt.gz" # main table
...
counts = counts[,grepl("Nippo_IP_polyP|Nippo_adult_polyP", colnames(counts))]
...
plotMDS(dge , col=colors[dge$samples$group], 
        pch = 16, cex=3,main="MDS plot of Nippostrongylus barasiliensis exWAGO vs Adult")
plot.new()
...
contrast = makeContrasts("Nippo_IP_polyP-Nippo_adult_polyP",levels = dge$design)
...
write.table(topTable, paste0(outPath_1, "nippostrongylus_brasiliensis.exwago_ip_vs_total_fdr05_lfc1_tmm_topTbl.txt"), 
...
de_tbl$exp = "N. brasiliensis IP vs Adult total"
de_tbl$exp = factor(de_tbl$exp)
levels(de_tbl$exp) = c(expression(paste(italic("A. ceylanicum "),
                                        "exWAGO IP vs Adult total")))

write.table(de_tbl ,paste0(outPath_1, "nippostrongylus_brasiliensis.exwago_ip_vs_total_fdr05_lfc1_tmm_deTbl.txt"), 
            col.names = T, row.names = T, sep = "\t", quote = F)
...
p = ggplot(de_tbl, aes(x=Ave_CPM, y=logFC, colour = col)) +
  ...
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), 
                           "inches"))
p
saveRDS(dge,paste0(outPath_7,"nippostrongylus_brasiliensis.exwago_ip_adult_tmm_dge.Rds"))
...
write.table(df, paste0("~/storage/Data/ms_evo_exwago/analysis/de_tables/tmm_norm_cats/",
                       "nippostrongylus_brasiliensis.tmm_nomcats.txt"), sep = "\t", quote = F, col.names = T, row.names = T)
#############################################################################################################################################################
...
outToptable = paste0(outPath_2, "nippostrongylus_brasiliensis.other_rRNA_topTbl.txt")
outToptable = sub("other", "exwago_IP_adult_lfc1_fdr01" , outToptable)
...
levels(de_tbl_norm$exp) <- c(expression(paste(italic("N. brasiliensis "))))


outDEtbl = paste0(outPath_2, "nippostrongylus_brasiliensis.exwago_IP_adult_lfc1_fdr01_rRNA_deTbl.txt")
...
saveRDS(dge,paste0(outPath_3, "nippostrongylus_brasiliensis.ip_vs_adult_rRNA_norm_dge.Rds"))
...
write.table(cameraRes, 
            paste0(outPath_4,"nippostrongylus_brasiliensis.IP_camera_enr_class_simple_rRNA.txt"),
...
write.table(cameraRes, 
            paste0(outPath_5,"nippostrongylus_brasiliensis.IP_camera_enr_family_simple_rRNA.txt"),
            sep = "\t", row.names = T, col.names = T, quote = F)
