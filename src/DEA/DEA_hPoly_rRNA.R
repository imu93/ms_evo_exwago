...
inPath_2 = "~/storage/Data/ms_evo_exwago/evo_exwago_guides/rRNA_tables/"
inFile_1 = "heligmosomoides_polygyrus.counts.1827.txt.gz" # main table
...
counts = counts[,grepl("Hpoly_IP_polyP|Hpoly_adult_polyP", colnames(counts))]
counts = counts[,!grepl("c42|c7", colnames(counts))]
...
plotMDS(dge , col=colors[dge$samples$group], 
        pch = 16, cex=3,main="MDS plot of Heligmosomoides polygyrus exWAGO vs Adult")
plot.new()
...
contrast = makeContrasts("Hpoly_IP_polyP-Hpoly_adult_polyP",levels = dge$design)
...
write.table(topTable, paste0(outPath_1, "heligmosomoides_polygyrus.exwago_ip_vs_total_fdr05_lfc1_tmm_topTbl.txt"), 
...
de_tbl$exp = "H. polygyrus IP vs Adult total"
de_tbl$exp = factor(de_tbl$exp)
levels(de_tbl$exp) = c(expression(paste(italic("H. polygyrus "),
                                        "exWAGO IP vs Adult total")))

write.table(de_tbl ,paste0(outPath_1, "heligmosomoides_polygyrus.exwago_ip_vs_total_fdr05_lfc1_tmm_deTbl.txt"), 
            col.names = T, row.names = T, sep = "\t", quote = F)
...
p = ggplot(de_tbl, aes(x=Ave_CPM, y=logFC, colour = col)) +
  ...
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), 
                           "inches"))
p
saveRDS(dge,paste0(outPath_7,"heligmosomoides_polygyrus.exwago_ip_adult_tmm_dge.Rds"))
...
write.table(df, paste0("~/storage/Data/ms_evo_exwago/analysis/de_tables/tmm_norm_cats/",
                       "heligmosomoides_bolygyrus.tmm_nomcats.txt"), sep = "\t", quote = F, col.names = T, row.names = T)
...
outToptable = paste0(outPath_2, "heligmosomoides_polygyrus.exwago_IP_adult_lfc1_fdr01_rRNA_topTbl.txt")
...
de_tbl_norm$exp = "H. polygyrus IP vs Adult total"
de_tbl_norm$exp <- factor(de_tbl_norm$exp)
ncont = "IP vs Adult total"
levels(de_tbl_norm$exp) <- c(expression(paste(italic("H. polygyrus "))))


outDEtbl = paste0(outPath_2, "heligmosomoides_polygyrus.exwago_IP_adult_lfc1_fdr01_rRNA_deTbl.txt")
...
saveRDS(dge,paste0(outPath_3, "heligmosomoides_polygyrus.ip_vs_adult_rRNA_norm_dge.Rds"))
...
write.table(cameraRes, 
            paste0(outPath_4,"heligmosomides_polygyrus.IP_camera_enr_class_simple_rRNA.txt"),
...
write.table(cameraRes, 
            paste0(outPath_5,"heligmosomides_polygyrus.IP_camera_enr_family_simple_rRNA.txt"),
            sep = "\t", row.names = T, col.names = T, quote = F)
