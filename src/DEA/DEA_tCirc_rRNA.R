...
inFile_1 = "teladorsagia_circumcincta.counts.1827.txt.gz" # main table
...
counts = counts[,grepl("Tc_IP_polyP|Tc_adult_polyP", colnames(counts))]
colnames(counts)
# Define groups
...
plotMDS(dge , col=colors[dge$samples$group], 
        pch = 16, cex=3,main="MDS plot of teladorsagia_circumcincta exWAGO vs Adult")
plot.new()
...
contrast = makeContrasts("Tc_IP_polyP-Tc_adult_polyP",levels = dge$design)
...
write.table(topTable, paste0(outPath_1, "teladorsagia_circumcincta.exwago_ip_vs_total_fdr05_lfc1_tmm_topTbl.txt"), 
...
de_tbl$exp = "T. circumcincta IP vs Adult total"
de_tbl$exp = factor(de_tbl$exp)
levels(de_tbl$exp) = c(expression(paste(italic("T. circumcincta "),
                                        "exWAGO IP vs Adult total")))

write.table(de_tbl ,paste0(outPath_1, "teladorsagia_circumcincta.exwago_ip_vs_total_fdr05_lfc1_tmm_deTbl.txt"), 
            col.names = T, row.names = T, sep = "\t", quote = F)
...
p = ggplot(de_tbl, aes(x=Ave_CPM, y=logFC, colour = col)) +
  ...
  theme(plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), 
                           "inches"))
p
saveRDS(dge,paste0(outPath_7,"teladorsagia_circumcincta.exwago_ip_adult_tmm_dge.Rds"))
...
write.table(df, paste0("~/storage/Data/ms_evo_exwago/analysis/de_tables/tmm_norm_cats/",
                       "teladorsagia_circumcincta.tmm_nomcats.txt"), sep = "\t", quote = F, col.names = T, row.names = T)
...
outToptable = paste0(outPath_2, "teladorsagia_circumcincta.exwago_IP_adult_lfc1_fdr01_rRNA_topTbl.txt")
...
de_tbl_norm$exp = "T. circumcincta IP vs Adult total"
de_tbl_norm$exp <- factor(de_tbl_norm$exp)
levels(de_tbl_norm$exp) <- c(expression(paste(italic("T. circumcincta "),
                                              ncont)))


outDEtbl = paste0(outPath_2, "teladorsagia_circumcincta.exwago_IP_adult_lfc1_fdr01_rRNA_deTbl.txt")
...
saveRDS(dge,paste0(outPath_3, "teladorsagia_circumcincta.ip_vs_adult_rRNA_norm_dge.Rds"))
...
write.table(cameraRes, 
            paste0(outPath_4,"teladorsagia_circumcincta.IP_camera_enr_class_simple_rRNA.txt"),
...
write.table(cameraRes, 
            paste0(outPath_5,"teladorsagia_circumcincta.IP_camera_enr_family_simple_rRNA.txt"),
            sep = "\t", row.names = T, col.names = T, quote = F)
